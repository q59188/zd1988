...
<div class="row">
    <div class="col-lg-8 mx-auto">
        <div class="card">
            ...
            <div class="card-body">
                <form class="form-horizontal" action="{{ route('smart_order.store') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <input type="text" class="aiz-date-range form-control" name="date_range" placeholder="{{ translate('Order Time') }}"
                                   data-format="YYYY-MM-DD" data-separator=" ~ " data-advanced-range="false" autocomplete="off" required>
                        </div>
                    </div>
                    <div class="form-group row">
                        ...
                        <div class="col-md-9">
                            <input type="text" placeholder="{{translate('Order Quantity')}}" id="quantity" name="quantity" class="form-control" required="">
                        </div>
                    </div>
                    <div class="form-group row">
                        <label class="col-md-3 col-form-label"> {{translate('Price Range')}} <span class="text-danger">*</span></label>
                        <div class="col-md-9">
                            <input style="width: 100px;float: left;" type="text" placeholder="{{translate('Min Price')}}"
                            ...
                            <input style="width: 100px;float: left;" type="text" placeholder="{{translate('Max Price')}}"
            ...
            <div class="modal-content">
                ...
                <div class="modal-body px-3 pt-3">
                    <div class="col-md-9">
                        ...
                        <button class="btn btn-primary" onclick="c_search()">search</button></div>
                    <form class="" enctype="multipart/form-data">
                        @foreach($customers as $kv)
                            <div style="width: 33%;float: left;" class="custom_n" tip="{{$kv->email}}" uname="{{$kv->name}}">
                                <input type="checkbox" name="user" value="{{$kv->id}}">{{$kv->name}} ({{$kv->email}})
                            </div>
                        @endforeach
            ...
            <div class="modal-content">
                ...
                <div class="modal-body px-3 pt-3">
                    <div class="col-md-9">
                        ...
                        <button class="btn btn-primary" onclick="s_search()">search</button></div>
                    <form class="" enctype="multipart/form-data">
                        @foreach($sellers as $vo)
                            <div style="width: 33%;float: left;" class="seller_n" tip="{{$vo->email}}" uname="{{$vo->seller_name}}">
                                <input type="radio" name="seller_name" value="{{$vo->user_id}}">{{$vo->seller_name}}({{$vo->email}})
                            </div>
                        @endforeach
                        <div class="text-right mt-4">
