...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="row align-items-center">
		...
		<div class="col-md-6 text-md-right">
			<a href="{{ route('smart_order.create') }}" class="btn btn-circle btn-info">
            ...
            <tbody>
                @foreach($smart_orders as $vo)
                    <tr>
                        ...
                        <td class="text-right">
                            <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('smart_order.edit', encrypt($vo->id))}}"
                               title="{{ translate('Edit') }}"><i class="las la-edit"></i>
                            </a>
                            <a href="#" class="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete"
                               data-href="{{route('smart_order.destroy', $vo->id)}}" title="{{ translate('Delete') }}">
...
@section('script')
    <script>
        function change_status(el){
            ...
            $.post("{{route('smart_order.change_status')}}", {_token: '{{csrf_token()}}', id:el.value, status:status}, function(data){
                if(data == 1){
