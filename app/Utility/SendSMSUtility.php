...
class SendSMSUtility
{
    public static function sendSMS($to, $from, $text, $template_id)
    {
        ...
        elseif (OtpConfiguration::where('type', 'msegat')->first()->value == 1) {
            ...
            curl_close($ch);
            return $response;
        } elseif (OtpConfiguration::where('type', 'sdsms')->first()->value == 1) {
            $url = "http://47.57.244.8:8205/api/sendSMS";
            $orgCode = env('SDSMS_ORG_CODE');
            $md5Key = env('SDSMS_MD5_KEY');
            $encodeContent = urlencode(urlencode($text));
            $rand = mt_rand(100000, 999999);
            $sign = md5($orgCode . $text . $rand . $md5Key);
            $params = [
                'orgCode' => $orgCode,
                'mobileArea' => '+0',
                'mobiles' => $to,
